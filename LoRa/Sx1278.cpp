...
#define IRQ_NUMBER 12  // Replace with the actual IRQ number for your interrupt
*/
// Function to check if interrupts are enabled globally
...
int LoRaClass::begin(long frequency, int SPI)
{
	...
	if (Init < 0)
	{
		cout << "Jetgpio initialisation failed. Error code: "<<Init << endl;
		exit(Init);
	}
	else{
   ...
   // configure DIO0
	int dio0_status = gpioSetMode(_dio0, JET_INPUT);
    gpioWrite(_dio0, 0);

	if (dio0_status < 0)
	{
		cout << "gpio setting up failed. Error code: " << dio0_status << endl;

		exit(Init);
	}
	else
	{
		cout << "Setting up dio0 sucessfull" << endl;
	}

    //configure SS
	
	int nss_status = gpioSetMode(_ss, JET_OUTPUT);
    gpioWrite(_ss, 1);

	if (nss_status < 0)
	{
		cout << "gpio setting up failed. Error code: " << nss_status << endl;

		exit(Init);
	}
	else
	{
		cout << "Setting up nss sucessfull" << endl;
	}
    //pinMode(_ss, OUTPUT);
    //digitalWrite(_ss, HIGH);

	int reset_status = gpioSetMode(_reset, JET_OUTPUT);

	if (reset_status < 0)
	{
		cout << "gpio setting up failed. Error code: " << reset_status << endl;

		exit(Init);
	}
	else
	{
		cout << "Setting up reset sucessfull" << endl;
	}
...
// Create a spi connection  (default 1 Mhz)
	int SPI_init = spiOpen(0, 1000000, 0, 0, 8, 1, 1);
  	if (SPI_init < 0)
  	{
...
void LoRaClass::onReceive(void(*callback)(int))
{
  ...
  if ((callback) && (!_onTxDone)) {  //  create IRQ thread only if it is not already created by onTxDone
	...
	if (dio0_status < 0)
	{
		cout << "GPIO setting up dio0 failed. Error code: " << dio0_status << endl;
	}

    if (gpioSetISRFunc(_dio0, RISING_EDGE, 1000, &timestamp, &LoRaClass::onDio0Rise) <0) {
...
void LoRaClass::setSPIFrequency(uint32_t frequency)
{
  //_spiSettings = SPISettings(frequency, MSBFIRST, SPI_MODE0);
	...
	int SPI_init = spiOpen(_spi, _spi_frequency, 0, 0, 8, 1, 1);
	if (SPI_init < 0)
	{
		cout << "Can't set frequency !!" << endl; 
	}
}
...
void LoRaClass::onTxDone(void(*callback)())
{
	...
	if (dio0_status < 0)
	{
		cout << "Cant set gpio mode for dio0" << endl;
	}
